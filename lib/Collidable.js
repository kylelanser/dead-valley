...
define(["Vector"], function (Vector) {
  ...
  var speculativeContactRectifier = function (result, delta) {
    ...
    var relativeNormalVelocity = (we.velocity.subtract(they.velocity)).dotProduct(normal);
    ...
    // apply impulse
    we.velocity.translate(weImpulse);
    they.velocity.translate(theyImpulse);
  ...
  var checkCollision = function (other) {
    ...
    var thisDot  = point.subtract(this.position).dotProduct(normal);
    var otherDot = point.subtract(other.position).dotProduct(normal);
    ...
    var vab = this.pointVel(point.subtract(this.position)).subtract(other.pointVel(point.subtract(other.position)));
  ...
  var pointVel = function (worldOffset) {
    return worldOffset.normal()
                  .scale(this.velocity.rot * Math.PI / 180.0)
  ...
  var rigidBodyContactRectifier = function (result) {
    ...
    we.position.translate(wePart);
    they.position.translate(theyPart);
    ...
    var vab = we.pointVel(point.subtract(we.position)).subtract(they.pointVel(point.subtract(they.position)));
    ...
    if (we.isRigidBody || they.isRigidBody) {
      ...
      var ap  = point.subtract(we.position).normal();
      var bp  = point.subtract(they.position).normal();
      ...
      we.velocity.translate(n.multiply(j  / we.mass));
      they.velocity.translate(n.multiply(-j  / they.mass));

      // TODO make all rot into radians
      // this used to be 180 * but I / 5 to make the collisions less jumpy
      we.velocity.rot += 34 * (ap.dotProduct(n.multiply(j)) / we.inertia) / Math.PI;
      they.velocity.rot += 34 * (bp.dotProduct(n.multiply(-j)) / they.inertia) / Math.PI;
