...
define(["Vector", "Game", "GridNode", "World", "Progress", "models/Building", "BuildingMarshal", "Console"],
  ...
  var Map = function (gridWidth, gridHeight) {
    ...
    // false otherwise
    this.updateWorldCoords = function (position, coords) {
      var gridX = Math.floor((position.x - this.originOffsetX + this.submapOffsetX) / Game.gridSize);
      var gridY = Math.floor((position.y - this.originOffsetY + this.submapOffsetY) / Game.gridSize);
    ...
    this.updatePosition = function (delta) {
      this.submapOffsetX = Math.round(this.submapOffsetX + this.velocityX);
      this.submapOffsetY = Math.round(this.submapOffsetY + this.velocityY);
      this.originOffsetX = Math.round(this.originOffsetX + this.velocityX);
      this.originOffsetY = Math.round(this.originOffsetY + this.velocityY);

      if (this.velocityX || this.velocityY) {
        this.needsRender = true;

        Game.events.fireEvent('map scroll', Vector.create(this.velocityX, this.velocityY));
      ...
      for (var i = 0; i < totalNodes; i++) {
        ...
        while (sprite) {
          ...
          if (sprite.shouldSave) {
            ...
            sprite.position.translate(offset);
            sprites.push(sprite.toString());
            // reset
            sprite.position.translate(reverseOffset);
    ...
    this.mapWorkerCallback = function (e) {
      ...
      switch (data.type) {
        ...
        case 'newtiles':
          ...
          var position = Vector.create(data.position.x, data.position.y);
          var stuff = waitingSectionDownloads[position];
          if (!stuff) {
            Console.warn("nothing in waitingSectionDownloads for '"+position.toString()+"'");
            return;
          }
          delete waitingSectionDownloads[position];

          World.setSectionData(position, data);

          this.setTilesFromStrings(stuff.recipientTiles, strings, position);

          if (data.sprites) {
            this.addSectionSprites(data.sprites, position);
          }

          if (data.buildings) {
            this.addSectionBuildings(data.buildings, stuff.recipientTiles, position);
          }
    ...
    this.addSectionSprites = function (sprites, position) {
      Game.addSpritesFromStrings(sprites, position.multiply(this.sectionWidth));
    };

    // buildings: list of building JSON objects
    // tiles: the section tiles the building goes into
    // position: the position of the recipient section
    this.addSectionBuildings = function (buildings, tiles, position) {
      var offset = position.multiply(this.sectionWidth);
    ...
    this.keepInView = function (sprite) {
      var speedX = sprite.velocity.x * 0.8;
      var speedY = sprite.velocity.y * 0.8;
      ...
      var screenX = sprite.position.x + speedX - this.originOffsetX;
      var screenY = sprite.position.y + speedY - this.originOffsetY;

      if (screenX < hBorder) {
        this.velocityX = screenX - hBorder;
      } else if (screenX > Game.GameWidth - hBorder) {
        this.velocityX = hBorder + screenX - Game.GameWidth;
      }
      if (screenY < vBorder) {
        this.velocityY = screenY - vBorder;
      } else if (screenY > Game.GameHeight - vBorder) {
        this.velocityY = vBorder + screenY - Game.GameHeight;
      }
    };
