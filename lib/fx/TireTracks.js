...
define(['Game', 'Sprite'], function (Game, Sprite) {
  ...
  TireTracks.prototype.preMove = function () {
    if (this.dots.length < this.length &&
      ...
      var position = this.car.position.add(this.wheel.position);
      // make sure we're not too close to the last one
      if (!this.dots.length ||
          _.last(this.dots).subtract(position).magnitude() > width/2) {
        // add a new dot
        position.rot = (this.car.position.rot * Math.PI)/180; // convert to radians
        this.dots.push(position);
      }
    }
  };
