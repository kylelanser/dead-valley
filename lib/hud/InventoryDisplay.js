  ...
  InventoryDisplay.prototype = {
    ...
    tableEventHandlers: {
      drop: function (e, ui) {
        ...
        var positionX = Math.round((ui.offset.left - tablePos.left) / this.cellSize);
        var positionY = Math.round((ui.offset.top - tablePos.top) / this.cellSize);

        // clear current draggable if we have one
        clearCurrentDraggable();

        if (this.inventory.isAvailable(draggingItem, positionX, positionY)) {
          // successful drag!

          // add the item to the inventory
          this.inventory.addItem(draggingItem, positionX, positionY);
          ...
          item = this.inventory.singleItemOverlay(draggingItem, positionX, positionY);
          ...
          if (item && item.acceptsDrop(draggingItem)) {
            ...
            this.inventory.addItem(newItem, positionX, positionY);
          } else {
    ...
    renderItem: function (item) {
      ...
      displayNode.css({left:position.left + this.itemOffset.left, top:position.top + this.itemOffset.top});
    ...
    itemSingleClick: function (event) {
      var target = $(event.target).parentsUntil('td').addBack().filter('.inventory-item');
      var position = target.offset();
      this.clickDragStart(
        target.data('item'),
        {left:event.pageX - position.left, top:event.pageY - position.top},
        event
      );
    },
