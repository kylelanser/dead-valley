define(["Game", "SpriteModel", "Vector", "Collidable"],
  ...
  var Drop = function (inventoryItem, dropPos) {
    ...
    if (arguments.length > 1) {
      ...
      // because the position object doesn't exist
  ...
  Drop.prototype.setPosition = function (dropPos) {
    var occupied;
    var newPos = Game.dude.position.clone();

    // push it to the left or right depending on
    // which side it was dropped on
    newPos.x += dropPos.x < Game.dude.position.x ? -HORIZONTAL_OFFSET : HORIZONTAL_OFFSET;
    ...
    for (var i = 0; i < 2; i++) {
      ...
      newPos.x += 2 * (Game.dude.position.x - newPos.x);
    }
    ...
    if (occupied) {
      ...
      newPos = Game.dude.position.clone();
    }
  ...
  Drop.prototype.setItem = function (itemData, complete) {
    ...
    require(['inventory/'+itemData.clazz], function (DropClass) {
      ...
      // configure calls init, but we already have a configured position
      // yes this is a hack
      var oldPos = self.position.clone();

      // configure the object from the item
      self.configure(item);

      self.position.set(oldPos);
      oldPos.free();
