...
define(["Game",
  ...
  var Dude = function () {
    ...
    this.maxEhaustion           = MAX_EXHAUSTION;
    this.exhaustion             = 0;
  ...
  Dude.prototype.preMove = function (delta) {
    ...
    Reporter.walkedDistance(this.velocity.magnitude() * delta);
  ...
  Dude.prototype.updateTouchingList = function () {
    ...
    this.touching = _.reject(this.touching, function (sprite) {
      if (!this.visible ||
          (this.position.subtract(sprite.position).dotProduct(this.velocity) > 0 &&
  ...
  Dude.prototype.leaveCar = function () {
    var canidates, position, node, nearby, occupied;
    ...
    if (this.moved) {
      ...
      do {
        position      = canidates.shift();
        node     = Game.map.getNodeByWorldCoords(position.x, position.y);
        nearby   = node.nearby();
        occupied = _.any(nearby, function (sprite) {
          return sprite.visible && sprite.checkPointCollision(position);
        });
      } while (occupied && canidates.length);

      if (occupied) {
        position = canidates[0]; // just use the driver's side
      }
  ...
  Dude.prototype.aimTowardPoint = function (coords, setDirection) {
    ...
    if (coords) {
      ...
      if (setDirection) {
        this.direction = (coords.x - this.position.x < 0) ? LEFT : RIGHT;
      }
      var dir = coords.subtract(this.position);
  ...
  Dude.prototype.saveMetadata = function () {
    ...
    metadata.energy    = this.health;
    metadata.exhaustion= this.health;
    ...
    if (this.driving) {
      metadata.position = this.driving.position.clone().round();
    }
    return metadata;
  };
  ...
  Dude.prototype.takeDamage = function (damage, point, other, collision) {
    if (this.alive() &&
      ...
      BloodSplatter.splat(this.position.clone(), '#900', damage);
      ...
      if (this.health <= 0) {
        ...
        var dir = this.position.subtract(other.position).normalize().scale(5);
        this.position.translate(dir);
      }
    }
  };
  ...
  Dude.prototype.distanceFromOrigin = function () {
    ...
    return Math.round(sprite.position.subtract(Game.startPosition).magnitude() / 1584);
  };
  ...
  Dude.prototype.fireWeapon = function () {
    if (this.activeFirearm()) {
      ...
      if (firearm && this.aimPoint) {
        var muzzle = this.aimPoint.subtract(this.position).normalize().scale(5);

        if (firearm.fire(this.position.add(muzzle), this.aimPoint, this.direction)) {
          this.firing = true;
        }
      }
    }
