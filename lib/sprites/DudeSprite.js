...
define(["Game", "Sprite", "Sky"],
  ...
  DudeSprite.prototype.render = function (delta) {
    ...
    if (this.lightImage && Sky.isDark()) {
      ...
      context.translate(dude.position.x - map.originOffsetX - this.lightImage.width/2,
                        dude.position.y - map.originOffsetY - this.lightImage.height/2);
    ...
    if (dude.takingDamage && this.imageData) {
      context.save();
      context.translate(dude.position.x - map.originOffsetX - dude.center.x,
                        dude.position.y - map.originOffsetY - dude.center.y);
      if (dude.direction === LEFT) {
  ...
  DudeSprite.prototype.drawAimedArm = function (frame) {
    ...
    var x = dude.position.x - map.originOffsetX - dude.center.x;
    var y = dude.position.y - map.originOffsetY - dude.center.y;
