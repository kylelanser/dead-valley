define(["Vector",
  ...
  ZombieSprite.prototype.draw = function (delta) {
    ...
    if (model.walking) {
      this.walkingFrameCounter += delta * model.velocity.magnitude();
  ...
  ZombieSprite.prototype.render = function (delta) {
    ...
    if (model.takingDamage && this.imageData) {
      context.save();
      context.translate(model.position.x - map.originOffsetX - model.center.x,
                        model.position.y - map.originOffsetY - model.center.y);
